...
def train_model():
    ...
    model = YOLO(r"runs\detect\3d_print_monitor\yolov8s_centered_synthetic2\weights\best.pt") 
    ...
    results = model.train(
        ...
        # TRAINING DURATION
        epochs=35,             # 100 loops over the data
        ...
        mosaic=0.2,             # Stitches 4 images together (Simulates Global View)
        mixup=0.1,              # Blends images slightly (Prevents overfitting)
        degrees=5.0,           # Rotates images +/- 10 degrees (Simulates camera tilt)
